...
Using users was an assumption that I made from the instructions as a way to track favorites. The instructions didn't clearly state anything about using users or not so I made the assumption that I could use users as part of the application. I felt that using users made the application more extensible and was just a more proper way of tracking favorites.
...
This project I actually ended up using redux for all state (except error boundary container) and don't have any components/containers with local state. As mentioned above, it's easy to reason about state changes and logic when redux is used properly and I believe it makes the code more readable. It also makes it easier than having to refactor when sibling components want to access each others state and data.
